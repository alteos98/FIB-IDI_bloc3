...
void MyGLWidget::carregaShaders()
{
  ...
  matambLoc = glGetAttribLocation (program->programId(), "matambVS");
  // Obtenim identificador per a l'atribut “matdiff” del vertex shader
  matdiffLoc = glGetAttribLocation (program->programId(), "matdiffVS");
  // Obtenim identificador per a l'atribut “matspec” del vertex shader
  matspecLoc = glGetAttribLocation (program->programId(), "matspecVS");
  // Obtenim identificador per a l'atribut “matshin” del vertex shader
  matshinLoc = glGetAttribLocation (program->programId(), "matshinVS");
...
void MyGLWidget::colFocus () {
  colorFocus = glm::vec3 (0.8, 0.8, 0.8); //en SCA
  glUniform3fv(colFocusLoc, 1, &colorFocus[0]);
}

void MyGLWidget::posFocus () {
  posicioFocus = glm::vec3 (focusX, 1, 1); //en SCA
  glUniform3fv(posFocusLoc, 1, &posicioFocus[0]);
}

void MyGLWidget::llAmbient () {
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_F: { //canvi de focus (càmera i escena)
